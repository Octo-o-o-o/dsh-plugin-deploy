...
2. 事实表在 `.dsh-assistant/reference/`（钉 harness **0.1.1-rc.2** @ `b150a551b8d4`，2026-08-21；48 个 slot）。slot / 服务 / 事件 / 字段名只能来自事实表或该 commit 的源码，**禁止编造**；插件实际运行的 harness 版本若不同，以运行版本为准并重生成事实层（`bash ~/WorkSpace/DshPluginAssistant/tools/install-into.sh .` 刷新）。
